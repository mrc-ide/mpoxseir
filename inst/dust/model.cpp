...
public:
  ...
  size_t size() const {
    return shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_I + shared->dim_Id + shared->dim_Ir + shared->dim_N + shared->dim_R + shared->dim_S + 9;
  }
  std::vector<real_type> initial(size_t step, rng_state_type& rng_state) {
    std::vector<real_type> state(shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_I + shared->dim_Id + shared->dim_Ir + shared->dim_N + shared->dim_R + shared->dim_S + 9);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    state_next[1] = cases * is_same_week + odin_sum1<real_type>(internal.n_SEa.data(), 0, shared->dim_n_SEa);
    for (int i = 1; i <= shared->dim_S; ++i) {
      state_next[9 + i - 1] = S[i - 1] - internal.n_SEa[i - 1];
    }
...
dust::pars_type<model> dust_pars<model>(cpp11::list user) {
  ...
  shared->offset_variable_D = shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 9;
  shared->offset_variable_E = shared->dim_D + shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 9;
  shared->offset_variable_Ea = shared->dim_S + 9;
  shared->offset_variable_Eb = shared->dim_Ea + shared->dim_S + 9;
  shared->offset_variable_I = shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 9;
  shared->offset_variable_Id = shared->dim_Ea + shared->dim_Eb + shared->dim_Ir + shared->dim_S + 9;
  shared->offset_variable_Ir = shared->dim_Ea + shared->dim_Eb + shared->dim_S + 9;
  shared->offset_variable_N = shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_I + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 9;
  shared->offset_variable_R = shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_S + 9;
...
cpp11::sexp dust_info<model>(const dust::pars_type<model>& pars) {
  ...
  cpp11::writable::strings nms({"time", "cases", "deaths", "S_tot", "E_tot", "I_tot", "R_tot", "D_tot", "N_tot", "S", "Ea", "Eb", "Ir", "Id", "R", "D", "E", "I", "N"});
  ...
  dim[10] = cpp11::writable::integers({shared->dim_Ea});
  dim[11] = cpp11::writable::integers({shared->dim_Eb});
  ...
  index[11] = integer_sequence(shared->offset_variable_Eb + 1, shared->dim_Eb);
