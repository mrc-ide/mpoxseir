...
[[cpp11::register]]
SEXP dust_cpu_model_generic_vax_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
...
[[cpp11::register]]
SEXP dust_cpu_model_generic_vax_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_model_generic_vax_filter(SEXP ptr, SEXP time_end,
...
// [[dust::param(n_vaccination, has_default = FALSE, default_value = NULL, rank = 3, min = -Inf, max = Inf, integer = FALSE)]]
...
public:
  ...
  struct shared_type {
    ...
    int dim_n_SEa_1;
    int dim_n_SEa_2;
    int dim_n_vaccination;
    int dim_n_vaccination_1;
    int dim_n_vaccination_12;
    int dim_n_vaccination_2;
    int dim_n_vaccination_3;
    int dim_n_vaccination_t;
    int dim_n_vaccination_t_1;
    int dim_n_vaccination_t_2;
    ...
    int n_group;
    std::vector<real_type> n_vaccination;
    int n_vax;
  ...
  model_generic_vax(const dust::pars_type<model_generic_vax>& pars) :
    shared(pars.shared), internal(pars.internal) {
  }
  size_t size() const {
    return shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_I + shared->dim_Id + shared->dim_Ir + shared->dim_N + shared->dim_R + shared->dim_S + 14;
  }
  std::vector<real_type> initial(size_t step, rng_state_type& rng_state) {
    std::vector<real_type> state(shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_I + shared->dim_Id + shared->dim_Ir + shared->dim_N + shared->dim_R + shared->dim_S + 14);
    internal.initial_time = step;
    state[0] = internal.initial_time;
    state[1] = shared->initial_cases;
    state[2] = shared->initial_deaths;
    state[3] = shared->initial_vax_given_S;
    state[4] = shared->initial_vax_given_Ea;
    state[5] = shared->initial_vax_given_Eb;
    state[6] = shared->initial_vax_given_R;
    state[7] = shared->initial_S_tot;
    state[8] = shared->initial_E_tot;
    state[9] = shared->initial_I_tot;
    state[10] = shared->initial_R_tot;
    state[11] = shared->initial_D_tot;
    state[12] = shared->initial_N_tot;
    state[13] = shared->initial_total_vax;
    std::copy(shared->initial_S.begin(), shared->initial_S.end(), state.begin() + 14);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type vax_given_S = state[3];
    const real_type vax_given_Ea = state[4];
    const real_type vax_given_Eb = state[5];
    const real_type vax_given_R = state[6];
    const real_type * S = state + 14;
    ...
    const real_type total_vax = state[13];
    state_next[0] = (step + 1) * shared->dt;
    state_next[13] = total_vax + vax_given_S + vax_given_Ea + vax_given_Eb + vax_given_R;
    real_type is_same_week = fmodr<real_type>(step, shared->steps_per_week) > 0;
    for (int i = 1; i <= shared->dim_n_eligible; ++i) {
      internal.n_eligible[i - 1] = S[shared->dim_S_1 * 0 + i - 1] + Ea[shared->dim_Ea_1 * 0 + i - 1] + Eb[shared->dim_Eb_1 * 0 + i - 1] + R[shared->dim_R_1 * 0 + i - 1];
    }
    state_next[11] = odin_sum2<real_type>(D, 0, shared->dim_D_1, 0, shared->dim_D_2, shared->dim_D_1);
    ...
    state_next[8] = odin_sum2<real_type>(E, 0, shared->dim_E_1, 0, shared->dim_E_2, shared->dim_E_1);
    ...
    state_next[9] = odin_sum2<real_type>(I, 0, shared->dim_I_1, 0, shared->dim_I_2, shared->dim_I_1);
    ...
    state_next[12] = odin_sum2<real_type>(N, 0, shared->dim_N_1, 0, shared->dim_N_2, shared->dim_N_1);
    state_next[10] = odin_sum2<real_type>(R, 0, shared->dim_R_1, 0, shared->dim_R_2, shared->dim_R_1);
    state_next[7] = odin_sum2<real_type>(S, 0, shared->dim_S_1, 0, shared->dim_S_2, shared->dim_S_1);
    ...
    for (int i = 1; i <= shared->dim_n_vaccination_t_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vaccination_t_2; ++j) {
        internal.n_vaccination_t[i - 1 + shared->dim_n_vaccination_t_1 * (j - 1)] = (static_cast<int>(time) >= (shared->vaccination_campaign_length) ? shared->n_vaccination[shared->dim_n_vaccination_12 * (shared->vaccination_campaign_length - 1) + shared->dim_n_vaccination_1 * (j - 1) + i - 1] : shared->n_vaccination[shared->dim_n_vaccination_12 * (time - 1) + shared->dim_n_vaccination_1 * (j - 1) + i - 1]);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_vaccination_t_Ea_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vaccination_t_Ea_2; ++j) {
        internal.n_vaccination_t_Ea[i - 1 + shared->dim_n_vaccination_t_Ea_1 * (j - 1)] = dust::math::min(dust::math::floor(internal.n_vaccination_t[shared->dim_n_vaccination_t_1 * (j - 1) + i - 1] * Ea[shared->dim_Ea_1 * 0 + i - 1] / (real_type) internal.n_eligible[i - 1]), Ea[shared->dim_Ea_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_vaccination_t_Eb_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vaccination_t_Eb_2; ++j) {
        internal.n_vaccination_t_Eb[i - 1 + shared->dim_n_vaccination_t_Eb_1 * (j - 1)] = dust::math::min(dust::math::floor(internal.n_vaccination_t[shared->dim_n_vaccination_t_1 * (j - 1) + i - 1] * Eb[shared->dim_Eb_1 * 0 + i - 1] / (real_type) internal.n_eligible[i - 1]), Eb[shared->dim_Eb_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_vaccination_t_R_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vaccination_t_R_2; ++j) {
        internal.n_vaccination_t_R[i - 1 + shared->dim_n_vaccination_t_R_1 * (j - 1)] = dust::math::min(dust::math::floor(internal.n_vaccination_t[shared->dim_n_vaccination_t_1 * (j - 1) + i - 1] * R[shared->dim_R_1 * 0 + i - 1] / (real_type) internal.n_eligible[i - 1]), R[shared->dim_R_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_vaccination_t_S_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vaccination_t_S_2; ++j) {
        internal.n_vaccination_t_S[i - 1 + shared->dim_n_vaccination_t_S_1 * (j - 1)] = dust::math::min(dust::math::floor(internal.n_vaccination_t[shared->dim_n_vaccination_t_1 * (j - 1) + i - 1] * S[shared->dim_S_1 * 0 + i - 1] / (real_type) internal.n_eligible[i - 1]), S[shared->dim_S_1 * (j - 1) + i - 1]);
      }
    }
    ...
    state_next[2] = deaths * is_same_week + odin_sum2<real_type>(internal.n_IdD.data(), 0, shared->dim_n_IdD_1, 0, shared->dim_n_IdD_2, shared->dim_n_IdD_1);
    ...
    state_next[4] = odin_sum2<real_type>(internal.n_vaccination_t_Ea.data(), 0, shared->dim_n_vaccination_t_Ea_1, 0, shared->dim_n_vaccination_t_Ea_2, shared->dim_n_vaccination_t_Ea_1);
    state_next[5] = odin_sum2<real_type>(internal.n_vaccination_t_Eb.data(), 0, shared->dim_n_vaccination_t_Eb_1, 0, shared->dim_n_vaccination_t_Eb_2, shared->dim_n_vaccination_t_Eb_1);
    state_next[6] = odin_sum2<real_type>(internal.n_vaccination_t_R.data(), 0, shared->dim_n_vaccination_t_R_1, 0, shared->dim_n_vaccination_t_R_2, shared->dim_n_vaccination_t_R_1);
    state_next[3] = odin_sum2<real_type>(internal.n_vaccination_t_S.data(), 0, shared->dim_n_vaccination_t_S_1, 0, shared->dim_n_vaccination_t_S_2, shared->dim_n_vaccination_t_S_1);
    ...
    state_next[1] = cases * is_same_week + odin_sum2<real_type>(internal.n_SEa.data(), 0, shared->dim_n_SEa_1, 0, shared->dim_n_SEa_2, shared->dim_n_SEa_1);
    ...
    for (int i = 1; i <= shared->dim_S_1; ++i) {
      for (int j = 1; j <= shared->dim_S_2; ++j) {
        state_next[14 + i - 1 + shared->dim_S_1 * (j - 1)] = S[shared->dim_S_1 * (j - 1) + i - 1] + internal.delta_S_n_vaccination[shared->dim_delta_S_n_vaccination_1 * (j - 1) + i - 1] - internal.n_SEa[shared->dim_n_SEa_1 * (j - 1) + i - 1];
      }
    }
...
namespace dust {
template<>
dust::pars_type<model_generic_vax> dust_pars<model_generic_vax>(cpp11::list user) {
  using real_type = typename model_generic_vax::real_type;
  auto shared = std::make_shared<model_generic_vax::shared_type>();
  model_generic_vax::internal_type internal;
  ...
  shared->dim_n_vaccination_1 = shared->n_group;
  shared->dim_n_vaccination_2 = shared->n_vax;
  shared->dim_n_vaccination_3 = shared->vaccination_campaign_length;
  shared->dim_n_vaccination_t_1 = shared->n_group;
  shared->dim_n_vaccination_t_2 = shared->n_vax;
  ...
  shared->dim_n_vaccination = shared->dim_n_vaccination_1 * shared->dim_n_vaccination_2 * shared->dim_n_vaccination_3;
  shared->dim_n_vaccination_12 = shared->dim_n_vaccination_1 * shared->dim_n_vaccination_2;
  shared->dim_n_vaccination_t = shared->dim_n_vaccination_t_1 * shared->dim_n_vaccination_t_2;
  ...
  internal.n_vaccination_t = std::vector<real_type>(shared->dim_n_vaccination_t);
  ...
  shared->n_vaccination = user_get_array_fixed<real_type, 3>(user, "n_vaccination", shared->n_vaccination, {shared->dim_n_vaccination_1, shared->dim_n_vaccination_2, shared->dim_n_vaccination_3}, NA_REAL, NA_REAL);
  shared->offset_variable_D = shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 14;
  shared->offset_variable_E = shared->dim_D + shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 14;
  shared->offset_variable_Ea = shared->dim_S + 14;
  shared->offset_variable_Eb = shared->dim_Ea + shared->dim_S + 14;
  shared->offset_variable_I = shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 14;
  shared->offset_variable_Id = shared->dim_Ea + shared->dim_Eb + shared->dim_Ir + shared->dim_S + 14;
  shared->offset_variable_Ir = shared->dim_Ea + shared->dim_Eb + shared->dim_S + 14;
  shared->offset_variable_N = shared->dim_D + shared->dim_E + shared->dim_Ea + shared->dim_Eb + shared->dim_I + shared->dim_Id + shared->dim_Ir + shared->dim_R + shared->dim_S + 14;
  shared->offset_variable_R = shared->dim_Ea + shared->dim_Eb + shared->dim_Id + shared->dim_Ir + shared->dim_S + 14;
  ...
  return dust::pars_type<model_generic_vax>(shared, internal);
}
template <>
cpp11::sexp dust_info<model_generic_vax>(const dust::pars_type<model_generic_vax>& pars) {
  const std::shared_ptr<const model_generic_vax::shared_type> shared = pars.shared;
  cpp11::writable::strings nms({"time", "cases", "deaths", "vax_given_S", "vax_given_Ea", "vax_given_Eb", "vax_given_R", "S_tot", "E_tot", "I_tot", "R_tot", "D_tot", "N_tot", "total_vax", "S", "Ea", "Eb", "Ir", "Id", "R", "D", "E", "I", "N"});
  ...
  dim[15] = cpp11::writable::integers({shared->dim_Ea_1, shared->dim_Ea_2});
  dim[16] = cpp11::writable::integers({shared->dim_Eb_1, shared->dim_Eb_2});
  dim[17] = cpp11::writable::integers({shared->dim_Ir_1, shared->dim_Ir_2});
  dim[18] = cpp11::writable::integers({shared->dim_Id_1, shared->dim_Id_2});
  dim[19] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2});
  dim[20] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2});
  dim[21] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2});
  dim[22] = cpp11::writable::integers({shared->dim_I_1, shared->dim_I_2});
  ...
  index[14] = integer_sequence(15, shared->dim_S);
  index[15] = integer_sequence(shared->offset_variable_Ea + 1, shared->dim_Ea);
  index[16] = integer_sequence(shared->offset_variable_Eb + 1, shared->dim_Eb);
  index[17] = integer_sequence(shared->offset_variable_Ir + 1, shared->dim_Ir);
  index[18] = integer_sequence(shared->offset_variable_Id + 1, shared->dim_Id);
...
using model_cpu = dust::dust_cpu<model_generic_vax>;
...
SEXP dust_cpu_model_generic_vax_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
                             ...
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config, cpp11::sexp ode_control) {
  return dust::r::dust_cpu_alloc<model_generic_vax>(r_pars, pars_multi, r_time, r_n_particles,
...
SEXP dust_cpu_model_generic_vax_simulate(SEXP ptr, cpp11::sexp r_time_end) {
...
SEXP dust_cpu_model_generic_vax_set_data(SEXP ptr, cpp11::list data,
                                       bool shared) {
