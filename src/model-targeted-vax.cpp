...
public:
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_target_met_t_1; ++i) {
      int j = 3;
      internal.target_met_t[i - 1 + shared->dim_target_met_t_1 * (j - 1)] = (odin_sum2<real_type>(N, 0, shared->dim_N_1, 2, 4, shared->dim_N_1) > dust::math::round(shared->prioritisation_strategy[shared->dim_prioritisation_strategy_1 * (j - 1) + i - 1] * shared->vaccination_coverage_target_1st_dose_prop * odin_sum2<real_type>(N, i - 1, i, 0, 4, shared->dim_N_1)) ? 1 : 0);
    }
    for (int i = 1; i <= shared->dim_target_met_t_1; ++i) {
      int j = 4;
      internal.target_met_t[i - 1 + shared->dim_target_met_t_1 * (j - 1)] = (odin_sum2<real_type>(N, 0, shared->dim_N_1, 3, 4, shared->dim_N_1) > dust::math::round(shared->prioritisation_strategy[shared->dim_prioritisation_strategy_1 * (j - 1) + i - 1] * shared->vaccination_coverage_target_2nd_dose_prop * odin_sum2<real_type>(N, i - 1, i, 0, 4, shared->dim_N_1)) ? 1 : 0);
    }
